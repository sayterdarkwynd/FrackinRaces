function init()
  ...
  if status.stat("isHerbivore")==1 or status.stat("isRobot")==1 or status.statPositive("isSugar")==1 then
...
function update(dt)
	 if status.stat("isCarnivore")==1 or status.stat("isOmnivore")==1 or status.stat("isRadien")==1 or status.stat("isMantizi")==1 then
	   applyEffects()
